...
#from Task_4.car_manager import Car
...
class TestCar(TestCase):
    ...
    def test_fuel_consumption_with_negative_or_zero_value_raises_exception(self):
        with self.assertRaises(Exception) as ex:
            self.car.fuel_consumption = 0

        self.assertEqual("Fuel consumption cannot be zero or negative!", str(ex.exception))

    def test_fuel_capacity_with_negative_or_zero_value_raises_exception(self):
        with self.assertRaises(Exception) as ex:
    ...
    def test_refuel_with_negative_or_zero_value_raises_exception(self):
        with self.assertRaises(Exception) as ex:
            self.car.refuel(0)
