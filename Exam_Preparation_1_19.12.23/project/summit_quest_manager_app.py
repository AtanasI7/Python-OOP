...
class SummitQuestManagerApp:
    ...
    def check_gear(self, climber_name: str, peak_name: str, gear: List[str]):
        ...
        missing_items = ', '.join(g for g in sorted(peak.get_recommended_gear()) if g not in gear)

        return (f"{climber_name} is not prepared to climb {peak_name}. "
                f"Missing gear: {missing_items}.")
    ...
    def get_statistics(self) -> str:
        ...
        total_peaks_climbed = len(set(p for c in climbers for p in c.conquered_peaks))
