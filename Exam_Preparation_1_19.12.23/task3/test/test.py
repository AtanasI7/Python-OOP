...
class TestClimbingRobot(TestCase):
    ...
    def setUp(self):
        ...
        self.robot_with_software.installed_software = [
            {"name": "PyCharm",
             "capacity_consumption": 50,
             "memory_consumption": 49},

            {"name": "Clion",
             "capacity_consumption": 49,
             "memory_consumption": 51}
        ]
    ...
    def test_category_when_not_in_allowed_categories_raises_value_error(self):
        with self.assertRaises(ValueError) as ve:
            self.robot.category = "Invalid"

        self.assertEqual(f"Category should be one of "
                         f"{self.ALLOWED_CATEGORIES}",
                         str(ve.exception))

    def test_used_capacity_expect_success(self):
    ...
    def test_used_memory_expect_success(self):
    ...
    def test_install_software_with_one_value_greater_than_max_values_returns_error_message(self):
        result = self.robot.install_software(
            {"name": "PyCharm",
             "capacity_consumption": 10,
    ...
    def test_install_software_with_both_value_greater_than_max_values_returns_error_message(self):
